...
function load_xmldsigjs() {
    ...
    __xmldisgjs_object = require('xmldsigjs');
    var xmldom = require('@xmldom/xmldom');
    require('xml-core').setNodeDependencies({
...
function new_WebSocket(url, options) {
    return new(require('ws'))(url, options);
}
            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode !== 200) {
                        ...
                        resolved_data = (JSON.parse(buf.toString()));
                    }
                } catch (err) {
                    abort(err);
                }
