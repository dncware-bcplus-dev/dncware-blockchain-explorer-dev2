...
function composeUint8Arrays(arr) {
    check_args({ uint8array: arr });
    var header = new Uint8Array(4 * (arr.length + 1));
    var dv = new DataView(header.buffer);
    dv.setUint32(0, arr.length, true);
    for (var i = 0; i < arr.length; i++) {
        dv.setUint32(4 * (i + 1), arr[i].length, true);
    }
...
function decomposeUint8Arrays(bytes) {
    if (bytes instanceof Uint8Array) {
        var { buffer, byteOffset, byteLength } = bytes;
    ...
    for (var i = 0; i < length; i++) {
        var l = dv.getUint32(4 * (i + 1), true);
        if (offset + l > byteLength) throw new RangeError();
        arr[i] = new Uint8Array(buffer, byteOffset + offset, l);
        offset += l;
    }
    if (byteLength !== offset) throw new RangeError();
    return arr;
}
var __xmldisgjs_object = null;

function load_xmldsigjs() {
    if (__xmldisgjs_object) return __xmldisgjs_object;
    __xmldisgjs_object = window.XmlDSigJs;
    if (!__xmldisgjs_object) {
        return (async function() {
            __xmldisgjs_object = await import('https://cdn.jsdelivr.net/npm/xmldsigjs@2.7.0/+esm');
            return __xmldisgjs_object;
        })();
    }
    return __xmldisgjs_object;
}

function load_domparser() {
    var DOMParser = window.DOMParser;
    if (!DOMParser) throw new Error('window.DOMParser not found');
    return DOMParser;
}

function new_WebSocket(url, options) {
...
async function callHTTP(method, url, bytes = new Uint8Array(0), options = {}, cancel) {
    check_args({ string: { method, url }, uint8array: { bytes } });
    return new Promise((resolve, reject) => {
        var xhr = new XMLHttpRequest();
        xhr.open(method, url);
        xhr.responseType = options.rawResponse ? 'arraybuffer' : 'json';
        xhr.onload = () => {
        ...
        function callback() {
            reject(new Error('XMLHttpRequest'));
            if (cancel) remove_cancel_listener(cancel, abort);
        }
    });
}
...
class Wallet_j extends BaseWallet {
    ...
    async _sign(bytes) {
        ...
        var XmlDSigJs = await load_xmldsigjs();
...
class SocketWS extends _SocketBase {
    ...
    _connect() {
        var ws = new_WebSocket(this.url, this.options);
        this.ws = ws;
                ...
                p.then(resp => {
                    ...
                    .catch(err => {
                        if (err instanceof RemoteError) {
                            var errinfo = ['RemoteError', err.message];
                        } else if (err instanceof Error) {
                            var errinfo = err.message;
                        } else {
                            var errinfo = String(err);
                        }
                        var text = JSON.stringify(errinfo);
                    ...
                    .then(async () => {
                        ...
                        var maxd = { c: 0, spv: -1, text: '["RemoteError","insufficient responses"]' };
                        ...
                        var errinfo = JSON.parse(maxd.text);
                        if (errinfo[0] === 'RemoteError') {
                            var err = new RemoteError(errinfo[1]);
                        } else {
...
async function wait_interval(ctx) {
    ...
    ctx.interval = Math.min(interval * 2, 1000);
    return true;
}
async function sssParamPeer(sss, { uw, cid, key, syncwait }, pid, cancel) {
    ...
    while (true) {
        ...
        ctx.syncwait -= Math.max(0, Date.now() - begin);
